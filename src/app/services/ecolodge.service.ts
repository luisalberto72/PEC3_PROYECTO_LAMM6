...
import { Ecolodge, EcolodgeResponse } from 'app/Model/ecolodge.model';
...
export class EcolodgeService {
  ...
  // Método para actualizar un ecolodge
  ...
  // Método para eliminar un ecolodge
  ...
  uploadImage(image: File, ecolodgeId: number): Observable<any> {
    ...
    return this.http.post(`http://localhost:8000/api/ecolodges/${ecolodgeId}/image`, formData);
  }

  filterAll(panelesSolares: number, energiaRenovable: number, role: string): Observable<Ecolodge[]> {
    const token = localStorage.getItem('user'); // Suponiendo que el token se guarda en el almacenamiento local
    ...
    params = params.append('paneles_solares', panelesSolares.toString());
    params = params.append('energia_renovable', energiaRenovable.toString());
    params = params.append('role', role);

    return this.http.get<Ecolodge[]>(this.apiUrl, { headers, params });
  }

  getEcolodgeDetails(id: number): Observable<any> {
    return this.http.get<any>(`/api/ecolodges/${id}`); 
  }
}
